...
async function getProfile(userId: string) {
  ...
  const res = await fetch(
    `${base}/api/stage10/profile/${encodeURIComponent(userId)}`,
    { cache: "no-store" }
  );
          ...
          >
            <div>
              ...
              <div style={{ marginTop: 6, opacity: 0.82 }}>
                <small>
                  {user
                    ? `${user.rank} • Lvl ${user.level} • ${user.xp} XP total`
                    : "Profile unavailable"}
                </small>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              ...
              {!mastery.length && (
                <div style={{ opacity: 0.78 }}>
                  <small>No mastery stats yet.</small>
                </div>
              )}
            </div>
          </div>
