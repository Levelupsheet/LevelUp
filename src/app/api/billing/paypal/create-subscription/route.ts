...
import { appBaseUrl, createPayPalSubscription, type PaidTier } from '@/lib/paypal';

export async function POST(req: Request) {
  try {
    const sessionUser = await getSessionUser();
    if (!sessionUser) return NextResponse.json({ ok: false, error: 'Please sign in before purchasing.' }, { status: 401 });
    const body = await req.json().catch(() => ({} as any));
    const rawPlan = String(body?.plan || '').toUpperCase();
    if (rawPlan !== 'PRO' && rawPlan !== 'PREMIUM') return NextResponse.json({ ok: false, error: 'Invalid plan.' }, { status: 400 });
