...
export default function PayPalSuccessClient({ orderId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setStatus("success");
        setMessage(
          `Payment complete. Your ${String(data?.subscriptionTier || "").toLowerCase()} plan is now active.`
        );
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
          ...
          <button
            onClick={() => router.replace("/start#pricing")}
            style={{
              padding: "10px 16px",
              borderRadius: 12,
              border: "1px solid rgba(255,255,255,0.16)",
