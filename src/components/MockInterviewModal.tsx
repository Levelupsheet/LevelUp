...
const STAGE_ONE_BANK: Record<Track, BossQuestion[]> = {
  ...
  aws: [
    ...
    { id: "aws2", prompt: "Enter the AWS CLI command to list S3 buckets", type: "cli_command", explanation: "Use aws s3 ls.", domainId: "aws", level: 1, data: { expectedCommands: ["aws s3 ls"], distractorCommands: ["aws s3api list-buckets", "aws ec2 describe-instances", "aws sts get-caller-identity"], placeholder: "Type an AWS CLI command", allowContains: true } },
  ...
  helpdesk: [
    ...
    { id: "hd2", prompt: "Enter the command to display IP configuration on Windows", type: "cli_command", explanation: "Use ipconfig or ipconfig /all.", domainId: "helpdesk", level: 1, data: { expectedCommands: ["ipconfig", "ipconfig /all"], distractorCommands: ["ping 8.8.8.8", "nslookup", "netstat -an"], placeholder: "Type a Windows networking command", allowContains: true } },
  ...
  desktop: [
    ...
    { id: "ds2", prompt: "Enter the command to check and repair protected Windows system files", type: "cli_command", explanation: "Use sfc /scannow.", domainId: "desktop", level: 1, data: { expectedCommands: ["sfc /scannow"], distractorCommands: ["chkdsk /f", "DISM /Online /Cleanup-Image /CheckHealth", "gpupdate /force"], placeholder: "Type a safe Windows repair command", allowContains: true } },
...
const STAGE_TWO_BANK: Record<Track, BossQuestion[]> = {
  azure_m365: [
    ...
    { id: "azs2_3", prompt: "Enter the PowerShell command to connect to Microsoft Graph", type: "cli_command", explanation: "Use Connect-MgGraph.", domainId: "azure", level: 2, data: { expectedCommands: ["Connect-MgGraph"], distractorCommands: ["Connect-AzAccount", "Get-MgUser", "az login"], placeholder: "Type a Microsoft Graph sign-in command", allowContains: true } },
  ],
  aws: [
    ...
    { id: "awss2_3", prompt: "Enter the AWS CLI command to identify the caller identity", type: "cli_command", explanation: "Use aws sts get-caller-identity.", domainId: "aws", level: 2, data: { expectedCommands: ["aws sts get-caller-identity"], distractorCommands: ["aws iam list-users", "aws s3 ls", "aws configure list"], placeholder: "Type an STS command", allowContains: true } },
  ],
  helpdesk: [
    ...
    { id: "hds2_3", prompt: "Enter the command to flush DNS on Windows", type: "cli_command", explanation: "Use ipconfig /flushdns.", domainId: "helpdesk", level: 2, data: { expectedCommands: ["ipconfig /flushdns"], distractorCommands: ["ipconfig /release", "nslookup", "ping 8.8.8.8"], placeholder: "Type a DNS repair command", allowContains: true } },
  ],
  desktop: [
...
export default function MockInterviewModal(props: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="modalOverlay" role="dialog" aria-modal="true">
      ...
      <div className="modal" style={{ maxWidth: step === "quiz" ? 1700 : 1540, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          {step === "quiz" && (
            <div className="modalBody" style={{ paddingTop: 8 }}>
              <DiabloQuizRunner
                ...
                media={{
                  playerIdleSrc: "/video/player-idle.mp4",
                  playerAttackSrc: "/video/player-attack.mp4",
                  enemyIdleSrc: "/video/enemy-idle.mp4",
                  enemyHitSrc: "/video/enemy-damage.mp4",
                  width: 1600,
                  height: 900,
                }}
              />
